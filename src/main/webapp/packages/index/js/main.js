...
angular.module("WebApp").run(["$rootScope", "settings", "$state", function($rootScope, settings, $state) {
    ...
    //获取用户的权限
    $rootScope.getFuncAuth = function(){
        var funcs = {};
        //获取权限树
        var funcTree = $rootScope.token.funcTree;
        //根节点
        funcs[funcTree.code] = funcTree.name;
        //获取children
        var rootChildren = funcTree.children;
        $.each(rootChildren , function(id , data){
            $rootScope.iterateTress(funcs , data);
        });
        $rootScope.token.funcs = funcs;
    };

    //递归获取用户的权限
    $rootScope.iterateTress = function(funcs , node){
        funcs[node.code] = node.name;
        $.each(node.children , function(id , data){
            $rootScope.iterateTress(funcs , data);
        });
    };
    ...
    $rootScope.reloadToken = function () {
        ...
        //设置未同步请求
        $.ajaxSettings.async = false;
        $.getJSON("packages/login/guest.json", function(data) {
            token = {"user": data.user, "funcTree": data.funcTree};
            photo = data.photo.data;
        });
        $.ajax({
            url: "mvc/login", async: false,
            dataType : 'json',
            data: { method: "GetLoginUser"},
            success: function (data) {
                if(data.code=='EC00'){
                    token = {"user": data.user, "funcTree": data.auth.funcTree};
                    //$rootScope.token.user.photo = data.photo.data;
                }
                $rootScope.token = token;
                $rootScope.token.user.photo = photo;
                $rootScope.getFuncAuth();
